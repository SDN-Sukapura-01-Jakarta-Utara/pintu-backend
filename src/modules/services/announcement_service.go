...
func NewAnnouncementService(repository repositories.AnnouncementRepository, r2Storage *utils.R2Storage) AnnouncementService {
...
// Create creates a new Announcement with file uploads to R2
func (s *AnnouncementServiceImpl) Create(gambar *multipart.FileHeader, files []*multipart.FileHeader, req *dtos.AnnouncementCreateRequest, userID uint) (*dtos.AnnouncementResponse, error) {
	// Parse tanggal
	...
	if gambar != nil {
		...
		// Upload gambar to R2
		fileKey, err := s.r2Storage.UploadFile(gambar, "pengumuman")
		if err != nil {
			return nil, err
		}
		gambarURL = fileKey
	...
	if len(files) > 0 {
		for _, file := range files {
			...
			// Upload file to R2
			fileKey, err := s.r2Storage.UploadFile(file, "pengumuman")
			if err != nil {
				return nil, err
			}
...
func (s *AnnouncementServiceImpl) GetByID(id uint) (*dtos.AnnouncementResponse, error) {
...
func (s *AnnouncementServiceImpl) GetAll(limit int, offset int) (*dtos.AnnouncementListResponse, error) {
	...
	return &dtos.AnnouncementListResponse{
		Data:   responses,
		Limit:  limit,
		Offset: offset,
		Total:  total,
...
func (s *AnnouncementServiceImpl) Update(id uint, gambar *multipart.FileHeader, files []*multipart.FileHeader, req *dtos.AnnouncementUpdateRequest, userID uint) (*dtos.AnnouncementResponse, error) {
	// Get existing data
	...
	if gambar != nil {
		...
		// Upload new gambar
		newFileKey, err := s.r2Storage.UploadFile(gambar, "pengumuman")
		if err != nil {
			return nil, err
		}
...
func (s *AnnouncementServiceImpl) Delete(id uint) error {
	// Get existing data
	existing, err := s.repository.GetByID(id)
	if err != nil {
...
func (s *AnnouncementServiceImpl) mapToResponse(data *models.Announcement) *dtos.AnnouncementResponse {
	// Map files from JSON
	...
	return &dtos.AnnouncementResponse{
		ID:              data.ID,
		Judul:           data.Judul,
