...
func (c *EkstrakurikulerController) GetAll(ctx *gin.Context) {
	var req dtos.EkstrakurikulerGetAllRequest
	if err := ctx.ShouldBindJSON(&req); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	...
	data, err := c.service.GetAllWithFilter(repositories.GetEkstrakurikulerParams{
		Filter: repositories.GetEkstrakurikulerFilter{
